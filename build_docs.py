    ...
    def run(self):
        ...
        try:
            ...
            for docbuilder in ["build_interface_docs.py"]:
                ...
                cmd = (
                    """%s -c 'import sys; sys.path.append("%s"); sys.path.append("%s"); execfile("%s", dict(__name__="__main__"))'"""
                    % (sys.executable, toolsdir, self.temp_install_dir, build_templates)
                )
                os.system(cmd)
        finally:
            os.chdir("..")
    ...
    class MyBuildDoc(BuildDoc):
        ...
        def zip_docs(self):
            ...
            try:
                ...
                warnings.warn(
                    "zlib not installed, storing the docs " "without compression"
                )
...
cmdclass = {"build_sphinx": MyBuildDoc, "api_docs": APIDocs, "clean": Clean}

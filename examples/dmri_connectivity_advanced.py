...
import warnings
from nipype.workflows.dmri.connectivity.nx import (
...
mapping.connect(
    [(FreeSurferSourceLH, mris_convertLHinflated, [("inflated", "in_file")])]
)
mapping.connect(
    [(FreeSurferSourceRH, mris_convertRHinflated, [("inflated", "in_file")])]
)
...
mapping.connect(
    [(eddycorrect, gen_WM_mask, [("outputnode.eddy_corrected", "in_file")])]
)
...
mapping.connect(
    [(eddycorrect, estimateresponse, [("outputnode.eddy_corrected", "in_file")])]
)
...
mapping.connect(
    [(csdeconv, probCSDstreamtrack, [("spherical_harmonics_image", "in_file")])]
)
mapping.connect([(probCSDstreamtrack, tracks2prob, [("tracked", "in_file")])])
mapping.connect(
    [(eddycorrect, tracks2prob, [("outputnode.eddy_corrected", "template_file")])]
)
...
mapping.connect(
    [(mri_convert_Brain, tck2trk, [("out_file", "registration_image_file")])]
)
...
mapping.connect(
    [(createnodes, creatematrix, [("node_network", "resolution_network_file")])]
)
"""
...
mapping.connect(
    [(creatematrix, CFFConverter, [("filtered_tractographies", "tract_files")])]
)
...
mapping.connect(
    [(creatematrix, networkx, [("intersection_matrix_file", "inputnode.network_file")])]
)

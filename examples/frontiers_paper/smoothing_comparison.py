...
iter_fwhm = pe.Node(interface=util.IdentityInterface(fields=["fwhm"]), name="iter_fwhm")
...
iter_smoothing_method.iterables = [
    ("smoothing_method", ["isotropic_voxel", "anisotropic_voxel", "isotropic_surface"])
]
...
preprocessing.connect(
    realign, "realigned_files", anisotropic_voxel_smooth, "inputnode.in_files"
)
preprocessing.connect(iter_fwhm, "fwhm", anisotropic_voxel_smooth, "inputnode.fwhm")
preprocessing.connect(
    compute_mask, "brain_mask", anisotropic_voxel_smooth, "inputnode.mask_file"
)
...
preprocessing.connect(
    surfregister, "out_reg_file", isotropic_surface_smooth, "reg_file"
)
...
preprocessing.connect(
    recon_all, "subjects_dir", isotropic_surface_smooth, "subjects_dir"
)

merge_smoothed_files = pe.Node(interface=util.Merge(3), name="merge_smoothed_files")
preprocessing.connect(
    isotropic_voxel_smooth, "smoothed_files", merge_smoothed_files, "in1"
)
preprocessing.connect(
    anisotropic_voxel_smooth, "outputnode.smoothed_files", merge_smoothed_files, "in2"
)
preprocessing.connect(
    isotropic_surface_smooth, "smoothed_file", merge_smoothed_files, "in3"
)
...
preprocessing.connect(
    iter_smoothing_method,
    ("smoothing_method", chooseindex),
    select_smoothed_files,
    "index",
)

rename = pe.MapNode(
    util.Rename(format_string="%(orig)s"), name="rename", iterfield=["in_file"]
)
...
contrastestimate.inputs.contrasts = [
    ("Task>Baseline", "T", ["Task-Odd", "Task-Even"], [0.5, 0.5])
]
...
main_workflow.connect(
    preprocessing,
    "select_smoothed_files.out",
    modelling,
    "specify_model.functional_runs",
)
main_workflow.connect(
    preprocessing, "compute_mask.brain_mask", modelling, "level1design.mask_image"
)

datasource = pe.Node(
    interface=nio.DataGrabber(infields=["subject_id"], outfields=["func", "struct"]),
    name="datasource",
)
datasource.inputs.base_directory = os.path.abspath("data")
datasource.inputs.template = "%s/%s.nii"
datasource.inputs.template_args = info = dict(
    func=[["subject_id", ["f3", "f5", "f7", "f10"]]], struct=[["subject_id", "struct"]]
)

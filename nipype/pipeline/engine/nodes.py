...
class Node(EngineBase):
    ...
    def set_input(self, parameter, val):
        ...
        logger.debug(
            "[Node] %s - setting input %s = %s", self.name, parameter, to_str(val)
        )
...
class MapNode(Node):
    ...
    def set_input(self, parameter, val):
        ...
        """
        logger.debug(
            "setting nodelevel(%s) input %s = %s", to_str(self), parameter, to_str(val)
        )
        self._set_mapnode_input(parameter, deepcopy(val))

    def _set_mapnode_input(self, name, newvalue):
        logger.debug(
            "setting mapnode(%s) input: %s -> %s", to_str(self), name, to_str(newvalue)
        )

...
def create_reg_workflow(name="registration"):
    ...
    binarize = pe.Node(fsl.ImageMaths(op_string="-nan -thr 0.5 -bin"), name="binarize")
    pickindex = lambda x, i: x[i]
    register.connect(fast, ("partial_volume_files", pickindex, 2), binarize, "in_file")
    """
    ...
    register.connect(mean2anatbbr, "out_matrix_file", outputnode, "func2anat_transform")
...
def get_subjectinfo(subject_id, base_dir, task_id, model_id):
    ...
    cond_file = os.path.join(
        base_dir, "models", "model%03d" % model_id, "condition_key.txt"
    )
    ...
    for idx in range(n_tasks):
        ...
        conds.append(
            [condition.replace(" ", "_") for condition in taskinfo[taskidx[0], 2]]
        )  # if 'junk' not in condition])
        files = sorted(
            glob(
                os.path.join(base_dir, subject_id, "BOLD", "task%03d_run*" % (idx + 1))
            )
        )
        runs = [int(val[-3:]) for val in files]
        run_ids.insert(idx, runs)
    json_info = os.path.join(
        base_dir,
        subject_id,
        "BOLD",
    ...
    if os.path.exists(json_info):
        ...
        with open(json_info, "rt") as fp:
            data = json.load(fp)
            TR = data["global"]["const"]["RepetitionTime"] / 1000.0
    else:
...
def analyze_openfmri_dataset(
    ...
    preproc.disconnect(
        preproc.get_node("plot_motion"),
        "out_file",
        preproc.get_node("outputspec"),
        "motion_plots",
    )
    """
    ...
    contrast_file = os.path.join(
        data_dir, "models", "model%03d" % model_id, "task_contrasts.txt"
    )
    ...
    def get_contrasts(contrast_file, task_id, conds):
        ...
        if os.path.exists(contrast_file):
            with open(contrast_file, "rt") as fp:
                contrast_def.extend(
                    [np.array(row.split()) for row in fp.readlines() if row.strip()]
                )
        contrasts = []
        for row in contrast_def:
            ...
            con = [
                row[1],
                "T",
                ["cond%03d" % (i + 1) for i in range(len(conds))],
    ...
    def sort_copes(copes, varcopes, contrasts):
        ...
        outcopes = all_copes.reshape(
            int(len(all_copes) / num_copes), num_copes
        ).T.tolist()
        outvarcopes = all_varcopes.reshape(
            int(len(all_varcopes) / num_copes), num_copes
        ).T.tolist()
    ...
    def get_subs(subject_id, conds, run_id, model_id, task_id):
        ...
        subs.append(
            ("bold_dtype_mcf_mask_smooth_mask_gms_tempfilt_mean_flirt", "affine")
        )
        ...
        for i in range(len(conds)):
            ...
            subs.append(
                (
                    "_warpall%d/varcope1_warp." % (len(conds) + i),
                    "varcope%02d." % (i + 1),
                )
            )
            subs.append(
                (
                    "_warpall%d/zstat1_warp." % (2 * len(conds) + i),
                    "zstat%02d." % (i + 1),
                )
            )
            ...
            subs.append(
                (
                    "_warpall%d/varcope1_trans." % (len(conds) + i),
                    "varcope%02d." % (i + 1),
                )
            )
            subs.append(
                (
                    "_warpall%d/zstat1_trans." % (2 * len(conds) + i),
                    "zstat%02d." % (i + 1),
                )
            )
    ...
    wf.connect(
        registration, "outputspec.func2anat_transform", datasink, "xfm.mean2anat"
    )
    wf.connect(
        registration, "outputspec.anat2target_transform", datasink, "xfm.anat2target"
    )
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="fmri_openfmri.py", description=__doc__)
    ...
    parser.add_argument("-m", "--model", default=1, help="Model index" + defstr)
    parser.add_argument(
        "-x", "--subjectprefix", default="sub*", help="Subject prefix" + defstr
    )
    parser.add_argument(
        "-t", "--task", default=1, type=int, help="Task index" + defstr  # nargs='+',
    )
    parser.add_argument(
        "--hpfilter",
        default=120.0,
    ...
    parser.add_argument("--fwhm", default=6.0, type=float, help="Spatial FWHM" + defstr)
    parser.add_argument(
    ...
    parser.add_argument(
        "--sd", dest="subjects_dir", help="FreeSurfer subjects directory (if available)"
    )
    parser.add_argument(
        "--target",
        dest="target_file",
    ...
    outdir = os.path.join(
        outdir, "model%02d" % int(args.model), "task%03d" % int(args.task)
    )

...
def get_info(dicom_files):
    ...
    meta = default_extractor(
        read_file(filename_to_list(dicom_files)[0], stop_before_pixels=True, force=True)
    )
    return (
        meta["RepetitionTime"] / 1000.0,
        meta["CsaImage.MosaicRefAcqTimes"],
...
def median(in_files):
    ...
    median_img = nb.Nifti1Image(average / float(idx + 1), img.affine, img.header)
    ...
    for idx, filename in enumerate(filename_to_list(motion_params)):
        ...
        if detrend_poly:
            ...
            for i in range(detrend_poly):
                X = np.hstack(
                    (X, legendre(i + 1)(np.linspace(-1, 1, timepoints))[:, None])
                )
    ...
    with open(out_ts_file, "wt") as fp:
        for fsindex in indices:
            ...
            for i0, row in enumerate(ts):
                fp.write(
                    "%d,%d,%d,%d," % (fsindex, ijk[0][i0], ijk[1][i0], ijk[2][i0])
...
def combine_hemi(left, right):
    ...
    indices = np.vstack(
        (
            1000000 + np.arange(0, lh_data.shape[0])[:, None],
            2000000 + np.arange(0, rh_data.shape[0])[:, None],
        )
    )
    all_data = np.hstack((indices, np.vstack((lh_data.squeeze(), rh_data.squeeze()))))
    filename = left.split(".")[1] + "_combined.txt"
    np.savetxt(
        filename, all_data, fmt=",".join(["%d"] + ["%.10f"] * (all_data.shape[1] - 1))
    )
...
def create_reg_workflow(name="registration"):
    ...
    binarize = Node(
        fs.Binarize(min=0.5, out_type="nii.gz", dilate=1), name="binarize_aparc"
    )
...
def create_workflow(
    ...
    filter1 = MapNode(
        fsl.GLM(
            out_f_name="F_mcart.nii.gz", out_pf_name="pF_mcart.nii.gz", demean=True
        ),
    ...
    wf.connect(realign, ("out_file", rename, "_filtermotart"), filter1, "out_res_name")
    ...
    wf.connect(filter1, ("out_res", rename, "_cleaned"), filter2, "out_res_name")
    ...
    warpall = MapNode(ants.ApplyTransforms(), iterfield=["input_image"], name="warpall")
    ...
    wf.connect(
        collector, ("out", get_names, "_avgwf.txt"), sampleaparc, "avgwf_txt_file"
    )
    wf.connect(
        collector, ("out", get_names, "_summary.stats"), sampleaparc, "summary_file"
    )
    ...
    samplerlh = MapNode(
        freesurfer.SampleToSurface(), iterfield=["source_file"], name="sampler_lh"
    )
    ...
    ts2txt.inputs.indices = (
        [8] + list(range(10, 14)) + [17, 18, 26, 47] + list(range(49, 55)) + [58]
    )
    ts2txt.inputs.label_file = os.path.abspath(
        ("OASIS-TRT-20_jointfusion_DKT31_CMA_labels_in_MNI152_" "2mm_v2.nii.gz")
    )
    ...
    substitutions += [
        ("_get_aparc_tsnr%d/" % i, "run%d_" % (i + 1)) for i in range(11)[::-1]
    ]
    ...
    wf.connect(
        registration, "outputspec.segmentation_files", datasink, "resting.mask_files"
    )
    ...
    wf.connect(createfilter1, "out_files", datasink, "resting.regress.@regressors")
    wf.connect(createfilter2, "components_file", datasink, "resting.regress.@compcorr")
    wf.connect(maskts, "out_file", datasink, "resting.timeseries.target")
    wf.connect(sampleaparc, "summary_file", datasink, "resting.parcellations.aparc")
    wf.connect(
        sampleaparc, "avgwf_txt_file", datasink, "resting.parcellations.aparc.@avgwf"
    )
    wf.connect(ts2txt, "out_file", datasink, "resting.parcellations.grayo.@subcortical")
    ...
    wf.connect(combiner, "out_file", datasink2, "resting.parcellations.grayo.@surface")
    return wf
...
def create_resting_workflow(args, name=None):
    ...
    if args.dicom_file:
        ...
        slice_times = (np.array(slice_times) / 1000.0).tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="target_file",
        help=(
            "Target in MNI space. Best to use the MindBoggle "
            "template - "
    ...
    parser.add_argument(
        "-o", "--output_dir", dest="sink", help="Output directory base", required=True
    )
    parser.add_argument(
        "-w", "--work_dir", dest="work_dir", help="Output directory base"
    )
    parser.add_argument(
        "-p", "--plugin", dest="plugin", default="Linear", help="Plugin to use"
    )
    parser.add_argument("--plugin_args", dest="plugin_args", help="Plugin arguments")

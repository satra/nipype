...
subject_id_infosource = pe.Node(
    util.IdentityInterface(fields=["subject_id"]), name="subject_id_infosource"
)
...
tbss_source = pe.Node(
    interface=nio.DataGrabber(outfiles=["fa_list", "md_list"]), name="tbss_source"
)
...
tbss_source.inputs.template_args = dict(
    fa_list=[["FA", subjects_list, "FA"]], md_list=[["MD", subjects_list, "MD"]]
)

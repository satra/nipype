...
inputnode = pe.Node(
    interface=util.IdentityInterface(fields=["func", "struct"]), name="inputspec"
)
"""
...
meanfuncmask = pe.Node(
    interface=fsl.BET(mask=True, no_output=True, frac=0.3), name="meanfuncmask"
)
...
threshold = pe.Node(
    interface=fsl.ImageMaths(out_data_type="char", suffix="_thresh"), name="threshold"
)
...
dilatemask = pe.Node(
    interface=fsl.ImageMaths(suffix="_dil", op_string="-dilF"), name="dilatemask"
)
...
def getinormscale(medianvals):
    return ["-mul %.10f" % (10000.0 / val) for val in medianvals]
...
highpass = pe.MapNode(
    interface=fsl.ImageMaths(suffix="_tempfilt"), iterfield=["in_file"], name="highpass"
)
...
smoothnode.iterables = ("fwhm", [5.0, 10.0])

...
inputnode = pe.Node(
    interface=util.IdentityInterface(fields=["dwi", "bvecs", "bvals", "subject_id"]),
    name="inputnode",
)
"""
...
createnodes.inputs.resolution_network_file = cmp_config.parcellation["freesurferaparc"][
...
mapping.connect(
    [
        (inputnode, image2voxel, [("dwi", "in_file")]),
        (inputnode, fsl2scheme, [("bvecs", "bvec_file"), ("bvals", "bval_file")]),
...
mapping.connect(
    [
        (track, camino2trackvis, [("tracked", "in_file")]),
...
mapping.connect(createnodes, "node_network", creatematrix, "resolution_network_file")
mapping.connect(
    [
        (
            FreeSurferSource,
            mri_convert_AparcAseg,
...
mapping.connect(
    [(mri_convert_AparcAseg, roigen_structspace, [("out_file", "aparc_aseg_file")])]
)

...
def extract_noise_components(
    realigned_file, mask_file, num_components=5, extra_regressors=None
):
    ...
    for filename in filename_to_list(mask_file):
        ...
        stdX[stdX == 0] = 1.0
        stdX[np.isnan(stdX)] = 1.0
        stdX[np.isinf(stdX)] = 1.0
...
def create_workflow(
    ...
    filter1 = MapNode(
        fsl.GLM(out_f_name="F_mcart.nii", out_pf_name="pF_mcart.nii", demean=True),
    ...
    wf.connect(
        slice_timing,
        ("timecorrected_files", rename, "_filtermotart"),
        filter1,
        "out_res_name",
    )
    ...
    wf.connect(createfilter2, "out_files", datasink, "resting.regress.@compcorr")
